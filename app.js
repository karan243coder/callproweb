...
async function uploadRecordingSegment(blob, segNum, isLast, overrideRoomId = null, overrideRole = null) {
    if (!blob || blob.size === 0) return;
    try {
        const rid = overrideRoomId || recordingSessionId || currentRoomId || 'unknown';
        const role = overrideRole || userRole || 'user';
        const formData = new FormData();
        const recExt = (blob.type && blob.type.includes('mp4')) ? 'mp4' : 'webm';

        // Include stable session id + role so repeated calls never clash.
        ...
        console.log(`✅ Segment ${segNum} uploaded (${(blob.size / 1024 / 1024).toFixed(1)} MB) for ${rid}`);
    } catch (e) {
...
function startNewSegment() {
    ...
    const sessionId = recordingSessionId || currentRoomId || 'unknown';
    const roleAtStart = userRole || 'user';
    recordedChunks = [];
    const mimeType = getSupportedMimeType();
    const recorder = new MediaRecorder(combinedStream, { mimeType, videoBitsPerSecond: 1500000, audioBitsPerSecond: 128000 });
...
function stopRecording() {
    ...
    recordingStopPromise = new Promise((resolve) => {
        ...
        const savedRoomId = recordingSessionId || currentRoomId || 'unknown';
        const savedRole = userRole || 'user';
        ...
        if (recorder && recorder.state !== 'inactive') {
            ...
            const finalMime = recorder.mimeType || getSupportedMimeType() || 'video/webm';
            recorder.onstop = async () => {
                try {
                    ...
                    console.error('Final recording upload failed:', e);
                    finishLog();
                } finally {
                    finishCleanup();
                }
            ...
            try { recorder.requestData(); } catch(e) {}
            try { recorder.stop(); } catch(e) { finishLog(); finishCleanup(); }
        } else {
            if (savedSegNum > 0) finishLog();
            finishCleanup();
        }
    });
...
function createRecordingSessionId(baseId = 'call') {
    const cleanBase = String(baseId || 'call').replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 42) || 'call';
    return `${cleanBase}_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}`;
}
...
async function initRoom(roomId, isCreator) {
    ...
    recordingSessionId = createRecordingSessionId(roomId);
...
async function callPeer(targetPeerId, callType = 'video') {
    ...
    const call = peer.call(targetPeerId, localStream, { metadata: { callType } });
    if (!call) {
    ...
    call.on('close', () => handleCallEnded('Call ended'));
    call.on('error', (err) => { console.error('Call error:', err); handleCallEnded('Call failed - recording saved'); });
    currentCall = call;
    bindCallAutoEnd(call);
...
async function handleIncomingCall(call) {
    ...
    currentCallMode = (call.metadata && call.metadata.callType) ? call.metadata.callType : 'video';
...
// ============ CALL AUTO-END + RECORDING SAFETY ============
function clearCallDisconnectTimer() {
    if (callDisconnectTimer) { clearTimeout(callDisconnectTimer); callDisconnectTimer = null; }
}

function scheduleCallAutoEnd(reason = 'Call disconnected', delay = 10000) {
    clearCallDisconnectTimer();
    callDisconnectTimer = setTimeout(() => {
        if (currentCall || isCallActive || mediaRecorder) {
            showToast(`${reason} - recording saved`);
            leaveRoom();
        }
    }, delay);
}

function handleCallEnded(message = 'Call ended') {
    clearCallDisconnectTimer();
    showToast(message);
    leaveRoom();
}

function bindCallAutoEnd(call) {
    if (!call || call._meetlinkAutoEndBound) return;
    call._meetlinkAutoEndBound = true;
    try {
        const pc = call.peerConnection;
        if (!pc) return;
        const checkState = () => {
            ...
            if (ice === 'connected' || ice === 'completed' || conn === 'connected') {
                clearCallDisconnectTimer();
            } else if (ice === 'failed' || conn === 'failed' || ice === 'closed' || conn === 'closed') {
                handleCallEnded('Call disconnected');
            } else if (ice === 'disconnected' || conn === 'disconnected') {
                scheduleCallAutoEnd('Network disconnected', 10000);
...
function showCallScreen(remoteStream) {
    ...
    // Start recording on BOTH ends; guard prevents duplicate/zombie recorder on re-stream events.
    if (recordingStartTimer) clearTimeout(recordingStartTimer);
    recordingStartTimer = setTimeout(() => {
        recordingStartTimer = null;
        if (!isLeavingRoom && currentCall && !isCallActive && !recordingStopPromise) startRecording();
    }, 2000);
}
...
async function leaveRoom() {
    ...
    const hadCallStart = !!callStartTime;
    const endedDuration = callStartTime ? formatCallDuration() : null;

    try {
        if (isCallActive || (mediaRecorder && mediaRecorder.state !== 'inactive') || recordingStopPromise) {
            await stopRecording();
        }
        if (hadCallStart) logEvent('call_ended', { duration: endedDuration, messages: messageCount });
        ...
        segmentNumber = 0; totalRecordingSize = 0; recordingSessionId = null;
        if (recordingTimer) { clearTimeout(recordingTimer); recordingTimer = null; }
...
function startFriendCall(friendUsername, callType = 'video') {
    ...
    // Set unique recording session ID for every new direct call.
    currentRoomId = createRecordingSessionId(currentUser.username + '_to_' + friendUsername);
    recordingSessionId = currentRoomId;
    userRole = 'creator';
...
function handleIncomingCyberCall(call) {
    ...
    cleanAccept.addEventListener('click', async () => {
        ...
        // Set unique recording session ID for every answered direct call.
        currentRoomId = createRecordingSessionId(call.peer + '_to_' + currentUser.username);
        recordingSessionId = currentRoomId;
        userRole = 'joiner';
